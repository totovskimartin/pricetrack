-- Drop existing discussions table and recreate it properly
...
-- Create discussions table with all required columns
CREATE TABLE public.discussions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    title VARCHAR(200) NOT NULL,
    slug VARCHAR(250) NOT NULL,
    content TEXT NOT NULL,
    category VARCHAR(50) NOT NULL DEFAULT 'Общи',
...
-- Insert some sample discussions
...
INSERT INTO public.discussion_comments (discussion_id, content, is_approved, created_by) VALUES
((SELECT id FROM public.discussions WHERE title = 'Добро качество на цени в Lidl тази седмица'), 'Съгласен съм! Вчера купих месо на много добра цена от Lidl. Качеството беше отлично.', true, (SELECT id FROM public.users WHERE role IN ('admin', 'super_admin') LIMIT 1)),
((SELECT id FROM public.discussions WHERE title = 'Съвети за пестене на пари при пазаруване'), 'Много полезни съвети! Аз също винаги правя списък и се придържам към него. Помага ми да не купувам ненужни неща.', true, (SELECT id FROM public.users WHERE role IN ('admin', 'super_admin') LIMIT 1)),
((SELECT id FROM public.discussions WHERE title = 'Кой супермаркет предлага най-добро качество месо?'), 'Препоръчвам Billa за месо. Имат добро качество и често имат промоции.', true, (SELECT id FROM public.users WHERE role IN ('admin', 'super_admin') LIMIT 1));
